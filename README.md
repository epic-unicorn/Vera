...
- **P2P Sync** — WhatsApp Web-style sharing: mobile → web viewer (no central server)
...
- Android SDK 21+ (Android), Xcode 12+ (iOS), or modern browser (Web)
...
```
lib/
├── core/              # Constants, errors, routing, platform detection
├── services/          # Vault (AES-256), Identity Shield (NER), Translation, Sync (WebRTC)
├── features/          # Auth, Blueprint, Decoder, Appointments, Sync
├── shared/            # UI widgets, theme (Material 3 WCAG AA), models
...
flutter test                          # Run all tests (135 passing ✅)
...
**Current Status:** 135 tests passing, 0 errors ✅

## Architecture Notes

- **Clean Architecture:** Feature-driven with clear separation (Domain, Data, Presentation)
- **State Management:** Riverpod providers for all business logic
- **Accessibility:** WCAG 2.1 Level AA (Atkinson Hyperlegible font, 48dp touch targets, 4.5:1+ contrast)
- **Platform Guards:** Services gracefully fail on unsupported platforms (e.g., vault on web)
- **Encryption:** All patient data encrypted at rest; P2P uses application-layer AES-256-GCM + DTLS
